...
module mac_1024_test(
    ...
    wire [1023:0] wrapped_eth_rx_data[0:3];
    wire [127:0] wrapped_eth_rx_keep[0:3];
    wire wrapped_eth_rx_valid[0:3], wrapped_eth_rx_ready[0:3], wrapped_eth_rx_last[0:3];

    generate
        for (j = 0; j < 4; j = j + 1)
        begin
            axis_dwidth_converter_64_1024 axis_dwidth_converter_64_1024_i(
                .aclk(eth_coreclk),
                .aresetn(~reset),
                ...
                .m_axis_tvalid(wrapped_eth_rx_valid[j]),
                .m_axis_tready(wrapped_eth_rx_ready[j]),
                .m_axis_tdata(wrapped_eth_rx_data[j]),
                .m_axis_tkeep(wrapped_eth_rx_keep[j]),
                .m_axis_tlast(wrapped_eth_rx_last[j])
            );
            ...
            egress_wrapper egress_wrapper_eth_i(
                ...
                .wrapped_data(wrapped_eth_rx_data[j ^ 1]),
                .wrapped_keep(wrapped_eth_rx_keep[j ^ 1]),
                .wrapped_last(wrapped_eth_rx_last[j ^ 1]),
                .wrapped_valid(wrapped_eth_rx_valid[j ^ 1]),
                .wrapped_ready(wrapped_eth_rx_ready[j ^ 1]),
