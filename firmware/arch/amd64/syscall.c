...
long open(const char *filename, long flags, long mode)
{
  return syscall3(SYS_open, (long)filename, flags, mode);
}

long fcntl(long fd, long cmd, void *arg)
{
  return syscall3(SYS_fcntl, fd, cmd, (long)arg);
}

long ioctl(long fd, long req, void *arg)
{
    return syscall3(SYS_ioctl, fd, req, (long)arg);
}

long gettimeofday(void *tv, void *tz)
{
...
long setresuid(long ruid, long euid, long suid)
{
	return syscall3(SYS_setresuid, ruid, euid, suid);
}

long setresgid(long rgid, long egid, long sgid)
{
